{ config, lib, pkgs, modulesPath, ... }:
{
	...
	options.local.services.acme."admin.prairiefire.ca".enable = lib.mkOption {
		type = lib.types.bool;
		default = false;
	};
	
	config = lib.mkIf config.local.services.acme."admin.prairiefire.ca".enable {
		
		security.acme.certs."admin.prairiefire.ca" = {
			domain = "admin.prairiefire.ca";
			extraDomainNames = [ "*.admin.prairiefire.ca" ];
