{ config, lib, pkgs, modulesPath, ... }:
{
	...
	config = lib.mkIf config.local.services.httpd."www.prairiefire.ca".enable {
		...
		local.services.acme."admin.prairiefire.ca".enable = lib.mkForce true;
		
		services.nginx = {

			# Unrestricted on VPN for updating.
			virtualHosts."admin.prairiefire.ca" = {
				forceSSL = true;
				useACMEHost = "admin.prairiefire.ca";
				globalRedirect = "www.admin.prairiefire.ca";
			};

			virtualHosts."www.admin.prairiefire.ca" = {
				forceSSL = true;
				useACMEHost = "admin.prairiefire.ca";
